...
def multi_process():
    ...
    spacer_info = util.read_tsv_ignore_N_line(WORK_DIR + IN + SPCR_ARR[0], deli_str=",")
    ...
    util.make_tsv(WORK_DIR + OU + "result_1_list.txt", header, result_1_list)

    header = ["Transposable element", "spacer (20 bp)", "index"]
    util.make_tsv(WORK_DIR + OU + "result_2_list.txt", header, result_2_list)
